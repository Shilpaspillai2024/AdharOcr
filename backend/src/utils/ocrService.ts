    ...
    if (/DOB|Date of Birth|Year of Birth|Male|Female/i.test(line)) {
      for (let j = i - 2; j <= i; j++) {
        if (j >= 0 && lines[j]) {
          ...
          if (
            cleaned &&
            /^[A-Z][a-zA-Z\s]{2,40}$/.test(cleaned) &&
...
const extractAddress = (text: string): string => {
  ...
  for (const line of lines) {
    ...
    if (
      ...
      trimmed.includes("D/O") ||
      trimmed.includes("S/O") ||
      trimmed.includes("W/O")
    ) {
  ...
  if (pincodeMatch && pincodeMatch.length > 1) {
    ...
    address = address.replace(
      new RegExp(`\\b${pincode}\\b.*?\\b${pincode}\\b`, "g"),
      pincode
    );
  }
...
const validateAadhaarText = (text: string): boolean => {
  const keywords = [
    "AADHAAR",
...
export const performOCR = async (
  ...
  try {
    ...
    const isAadhaar = validateAadhaarText(frontText);
    if (!isAadhaar) {
    ...
    if (backImagePath) {
      ...
      backText = backResult.data.text;
       ...
       const isBackAadhaar = validateAadhaarText(backText);
      if (!isBackAadhaar) {
        await worker.terminate();
        return { message: 'Back image does not appear to be an Aadhaar card' };
      }
